...
def child_reader(queue):
    ...
    try:
        ...
        shared_tensor = paddle.to_tensor(paddle.base.core.LoDTensor._new_shared_xpu(ipc_meta))
        print("[Child] Reconstructed tensor on", shared_tensor.place, "with value:", shared_tensor.numpy())
    except Exception as e:
    ...
    if not (shared_tensor.cpu() == expected).all().item():
        raise ValueError("Child: Reconstructed tensor does not match expected value!")
...
class TestXpuIpcSharing(unittest.TestCase):
    def test_ipc_share_read(self):
        ...
        print("[Parent] Created tensor on", tensor.place, "with value:", tensor.cpu().numpy())
