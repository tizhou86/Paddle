...
void Copy<phi::XPUPinnedPlace, phi::XPUPlace>(phi::XPUPinnedPlace dst_place,
  ...
  if (stream) {
    ...
    phi::RecordEvent record_event(
        "cudaMemcpySync:XPU->XPUPinned", phi::TracerEventType::UserDefined, 1);
    platform::cudaMemcpySync(dst, src, num, cudaMemcpyDeviceToHost);
  }
}
...
void Copy<phi::XPUPlace, phi::XPUPinnedPlace>(phi::XPUPlace dst_place,
  ...
  if (stream) {
    ...
    phi::RecordEvent record_event(
        "cudaMemcpySync:XPUPinned->XPU", phi::TracerEventType::UserDefined, 1);
    platform::cudaMemcpySync(dst, src, num, cudaMemcpyHostToDevice);
  }
}
//!!!end xpupinnedplace!!!
