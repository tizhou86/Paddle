...
/** 
 * Helper: Insert or retrieve a DeviceEvent in the map without default-constructing it.
...
static platform::DeviceEvent& GetOrCreateEvent(
  ...
  if (it == event_map.end()) {
    ...
    auto emplace_result = event_map.emplace(
...
AsyncLoad::Task::Task(const Place& place)
    : use_event_(!is_xpu_place(place)),
      ...
      load_event_(
          use_event_ ? place : phi::CPUPlace(),
          platform::GenerateDeviceEventFlag()),
      task_place_(place) {}

AsyncLoad::Task::~Task() = default;

bool AsyncLoad::Task::IsCompleted() {
  if (!use_event_) {
...
void AsyncLoad::Task::XpuSynchronize() {
  if (!use_event_) {
    return;
  }
...
std::shared_ptr<AsyncLoad::Task> AsyncLoad::Offload(phi::DenseTensor* dst,
                                                    const phi::DenseTensor& src) {
  PADDLE_ENFORCE_EQ(is_xpu_place(src.place()),
                    true,
  ...
  phi::memory_utils::Copy(cpu_place, dst_ptr,
                          src.place(), src_ptr,
                          size,
                          /*stream=*/nullptr);
...
std::shared_ptr<AsyncLoad::Task> AsyncLoad::OffloadWithOffset(
  ...
  PADDLE_ENFORCE_EQ(is_xpu_place(src.place()),
                    ...
                    phi::errors::InvalidArgument(
                        "OffloadWithOffset requires XPU source."));
  ...
  const void* src_ptr = static_cast<const char*>(src.data()) + src_offset * elem_size;
  void* dst_ptr = static_cast<char*>(dst->data()) + dst_offset * elem_size;

  phi::memory_utils::Copy(dst->place(), dst_ptr,
                          src.place(), src_ptr,
                          copy_bytes,
                          /*stream=*/nullptr);
...
/* ------------ Reload (CPU -> XPU) ------------ */
std::shared_ptr<AsyncLoad::Task> AsyncLoad::Reload(phi::DenseTensor* dst,
                                                   const phi::DenseTensor& src) {
  PADDLE_ENFORCE_EQ(
      is_initialized_,
      true,
  ...
  phi::memory_utils::Copy(xpu_place_, dst_ptr,
                          src.place(), src_ptr,
                          size,
                          /*stream=*/nullptr);
