...
void BindTensor(pybind11::module &m) {  // NOLINT
  ...
  framework_tensor
      ...
      .def("set",
           ...
           py::arg("zero_copy") = false,
           R"DOC(
        Set the data of Tensor on place with a given numpy array.

        Args:
          array (numpy.ndarray): The numpy array to set.
          place (CPUPlace|CUDAPlace|XPUPlace|IPUPlace|CUDAPinnedPlace): The target place.
          zero_copy (bool, optional): Whether to share memory with the input numpy array.
                                      (Only works with CPUPlace.) Default: False.
      ...
      .def(py::init([](const std::vector<std::vector<size_t>> &recursive_sequence_lengths) {
        LegacyLoD new_lod;
      ...
      .def(
          ...
          [](phi::DenseTensor &self,
             const std::vector<std::vector<size_t>> &recursive_sequence_lengths) {
            LegacyLoD new_lod;
            new_lod.reserve(recursive_sequence_lengths.size());
            std::copy(recursive_sequence_lengths.begin(),
                      recursive_sequence_lengths.end(),
            ...
            LegacyLoD new_offset_lod = ConvertToOffsetBasedLegacyLoD(new_lod);
            PADDLE_ENFORCE_EQ(
                CheckLegacyLoD(new_offset_lod, common::vectorize(self.dims()).front()),
                true,
                common::errors::InvalidArgument(
                    "The provided recursive_sequence_lengths info is invalid, "
                    "the LegacyLoD converted by recursive_sequence_lengths is %s",
           ...
           [](phi::DenseTensor &self) {
             if (!self.IsInitialized() || self.numel() == 0)
               throw std::runtime_error(
                   "Tensor not initialized or numel is 0. could not pass to shared memory.");
             auto *holder = dynamic_cast<memory::allocation::Allocation *>(
                 self.Holder().get());
             PADDLE_ENFORCE_EQ(
                 phi::is_gpu_place(holder->place()), true,
                 common::errors::InvalidArgument(
                     "Tensor is not on GPU. share_cuda only supports GPU tensors."));
      ...
      .def("_new_shared_cuda",
           [](py::tuple t) {
             if (t.size() != 7)
               throw std::runtime_error("Invalid Tensor meta info for shared cuda tensor!");
      ...
      .def("_share_buffer_with",
           [](phi::DenseTensor &self, const phi::DenseTensor src,
             ...
             VLOG(6) << "[DEBUG XPU] _share_buffer_with: current XPU device = " << dev_id;
             ...
             PADDLE_ENFORCE_NOT_NULL(
                 xpu_ipc_allocation,
                 common::errors::PreconditionNotMet(
                     "Tensor is not Xpu IPC shared tensor. "
                     "Now only Tensor shared by xpu ipc could use this "
                     "api."));
      ...
      .def("_share_xpu",
           [](phi::DenseTensor &self) {
             ...
             int dev_id = platform::GetXPUCurrentDeviceId();
             paddle::platform::SetXPUDeviceId(dev_id);
             VLOG(6) << "[DEBUG XPU] _share_xpu: current XPU device = " << dev_id;
             ...
             PADDLE_ENFORCE_EQ(
                 ...
                 common::errors::InvalidArgument(
                     "Tensor is not on XPU. share_xpu only supports XPU tensors."));
             void *base_ptr = holder->base_ptr();
             ptrdiff_t offset_bytes = reinterpret_cast<char *>(holder->ptr()) -
                                      reinterpret_cast<char *>(base_ptr);
             ...
             VLOG(6) << "[DEBUG XPU] _share_xpu: cudaIpcGetMemHandle returned: " << ret;
             PADDLE_ENFORCE_XPU_SUCCESS(ret);
             // Use the correct size for the IPC handle.
             auto _handle = py::bytes(reinterpret_cast<char *>(&handle),
                                      (py::ssize_t)sizeof(cudaIpcMemHandle_t));
             VLOG(6) << "[DEBUG XPU] _share_xpu: IPC handle (bytes) = " << _handle;
      ...
      .def("_new_shared_xpu",
           [](py::tuple t) {
             if (t.size() != 7)
               throw std::runtime_error("Invalid Tensor meta info for shared xpu tensor!");
             
             // Get the current device ID.
             int dev_id = platform::GetXPUCurrentDeviceId();
             paddle::platform::SetXPUDeviceId(dev_id);
             VLOG(6) << "[DEBUG XPU] _new_shared_xpu: current XPU device = " << dev_id;
             ...
             tensor.ResetHolderWithType(shared_holder,
                                        static_cast<phi::DataType>(t[3].cast<int>()));
             tensor.Resize(common::make_ddim(t[4].cast<std::vector<int>>()));
             VLOG(6) << "[DEBUG XPU] _new_shared_xpu: Reconstructed tensor dims: " << tensor.dims();
             return tensor;
           },
           R"DOC(
      ...
      .def("_share_filename",
           [](phi::DenseTensor &self, bool use_file_descriptor) {
             if (!self.IsInitialized() || self.numel() == 0)
               throw std::runtime_error("Tensor not initialized or numel is 0. Could not pass to shared memory.");
             ...
             PADDLE_ENFORCE_EQ(
                 ...
                 true, common::errors::InvalidArgument(
                           "Tensor is not on CPU. share_filename only supports CPU tensors."));
             ...
             if (mmap_allocation == nullptr) {
               ...
               if (FLAGS_use_shm_cache) {
                 find_id = memory::allocation::MemoryMapAllocationPool::Instance().FindFromCache(flags, data_size);
               }
               if (find_id != -1) {
                 handle = memory::allocation::MemoryMapAllocationPool::Instance().GetById(find_id).file_name_;
               }
               int shared_fd = -1;
               auto shared_holder =
           ...
           R"DOC(
           Serialize CPU lod tensor in shared memory to a tuple.
           If the tensor is not already in shared memory, it is copied first.

           Returns:
               tuple: (ipc name, shared fd, size, data type, dims, lod, use_file_descriptor)
           )DOC")
      .def("_new_shared_filename",
           [](py::tuple t) {
             ...
             int find_id = -1;
             if (FLAGS_use_shm_cache) {
               find_id = memory::allocation::MemoryMapAllocationPool::Instance().FindFromCache(flags, size, ipc_name, false);
             }
             auto shared_holder =
                 memory::allocation::AllocateRefcountedMemoryMapAllocation(
           ...
           Returns:
               A DenseTensor reconstructed from shared memory info.
           )DOC")
      .def("_shared_incref",
           [](phi::DenseTensor &self) {
             auto *mmap_allocation = dynamic_cast<
           ...
           R"DOC(
            Increase reference count of a share_filename tensor.
           )DOC")
      .def("_shared_decref",
           [](phi::DenseTensor &self) {
             auto *mmap_allocation = dynamic_cast<
           ...
           R"DOC(
            Decrease reference count of a share_filename tensor.
           )DOC")
      .def(py::pickle(
          [](const phi::DenseTensor &t) {  // __getstate__
            ...
            PADDLE_ENFORCE_EQ(phi::is_cpu_place(holder->place()), true,
                              common::errors::PreconditionNotMet(
                                  "Tensor is not on CPU. Only CPU tensors can be serialized."));
            ...
            PADDLE_ENFORCE_NOT_NULL(
                ...
                common::errors::PreconditionNotMet(
                    "Tensor is not in shared memory. Only tensors on shared memory can be serialized."));
