...
class DygraphShardingOptimizerV2:
    ...
    def _sharding_sync_parameters(self):
        ...
        with framework.no_grad():
            ...
            # Synchronize slice parameters back to original parameters
            for param in self._parameter_list:
                if param.dtype == paddle.float32:
                    slice_param = self._slice_params[param.name]
                    param.set_value(slice_param.value())
    ...
    def _create_slice_param(self, param):
        if param.dtype == paddle.float32:
            # Initialize slice parameter with the value of the original parameter
        ...
        if param.dtype == paddle.float32:
            slice_param.set_value(param.value())
        ...
        for comm_buffer in self._comm_buffer_list:
            ...
            for param in comm_buffer.params:
                ...
                if param.dtype == paddle.float32:
                    ...
                    if grad is not None:
                        slice_param._set_grad_ivar(grad)
                else:
